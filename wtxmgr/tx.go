...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32, change bool) (bool, error) {
	if block == nil {
		...
		v := valueUnminedCredit(xpcutil.Amount(rec.MsgTx.TxOut[index].Value), change)
	...
	txOutAmt := xpcutil.Amount(rec.MsgTx.TxOut[index].Value)
		...
		for i := range b.transactions {
			...
			if blockchain.IsCoinBaseTx(&rec.MsgTx) {
				...
				for i, output := range rec.MsgTx.TxOut {
					...
					if credKey != nil {
						minedBalance -= xpcutil.Amount(output.Value)
						err = deleteRawUnspent(ns, unspentKey)
						if err != nil {
							return err
						}
...
func (s *Store) UnspentOutputs(ns walletdb.ReadBucket) ([]Credit, error) {
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		cred := Credit{
			...
			Amount:       xpcutil.Amount(txOut.Value),
...
func (s *Store) Balance(ns walletdb.ReadBucket, minConf int32, syncHeight int32) (xpcutil.Amount, error) {
